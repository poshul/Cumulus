...
serviceNameLo="nimbus"                                  # service name with the first letter in lowercase
serviceName="Nimbus"                                    # service name
serviceUser="root"                                      # OS user name for the service
serviceGroup="root"                                    # OS group name for the service
applDir="/root/"                          # home directory of the service application
serviceUserHome="/$serviceUser"                       # home directory of the service user
serviceLogFile="/var/log/syslog"               # We send everything to syslog so that it can be accessed from the Amazon HUD
maxShutdownTime=15                                         # maximum number of seconds to wait for the daemon to terminate normally
pidFile="/var/run/$serviceNameLo.pid"                      # name of PID file (PID = process ID number)
javaCommand="java"                                         # name of the Java launcher without the path
javaExe="$javaCommand"                      # file name of the Java application launcher executable
javaArgs="-jar $applDir/nimbus.jar"                     # arguments for Java launcher
javaCommandLine="$javaExe $javaArgs"                       # command line to start the Java service application
javaCommandLineKeyword="nimbus.jar"                     # a keyword that occurs on the commandline, used to detect an already running service process and to distinguish it from others
